...
#the script here need the fasta header(dot_bracket file header) to have the word “ENERGY”. In
practice, user could have anything in the header. (could you make it just jump the first line?)

#USER: Input ct file
ctfileinput <- read.table("HCV_JFH1_2a_genome_model.ct")
#I think the script here need the CT file header to be in certain length. In practice, the user could have anything in the header. (could you make it just jump the first line?)
...
#shannoninput <- read_excel("shannon.txt", header = TRUE)
shannon <- shannoninput$Shannon
#Here the script require that the Shannon file has a header, most of Shannon files don’t have header, maybe we could change “header =FALSE”
...
#write.csv(ordered_results_table)
```
